...
from azure.ai.evaluation import (
    evaluate, ToolCallAccuracyEvaluator, IntentResolutionEvaluator, 
...
def run_simple_evaluation():
    ...
    # Azure AI Studio project info for evaluators that require it (must be a dict)
    ...
    if not all(azure_ai_project.values()):
        ...
        raise ValueError(
            "Missing Azure AI project settings in .env: "
            + ", ".join(missing)
            + ". Ensure AZURE_SUBSCRIPTION_ID, AZURE_RESOURCE_GROUP, and AI_FOUNDRY_PROJECT_NAME are set."
        )
    ...
    with open(eval_input_path, "w", encoding="utf-8") as f:        
        for i, row in enumerate(test_data, 1):
            ...
            tracer.start_as_current_span("example-queries")
    ...
    evaluators_config: Dict[str, Any] = {
